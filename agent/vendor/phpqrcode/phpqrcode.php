...
/*
 * 二维码编码器
 * config文件，合并后的verion调谐时间。
 */
define('QR_CACHEABLE', false);          // 使用高速缓存磁盘读取但较少的CPU功率，口罩和格式模板存储在
define('QR_CACHE_DIR', false);          // 当使用QR _ cacheable = = = true
define('QR_LOG_DIR', false);            // 默认错误日志目录
define('QR_FIND_BEST_MASK', true);      // 如果是true，估计最好的掩饰（规格默认，但极慢；设置为false来显著提高性能但（可能）糟糕的代码质量
define('QR_FIND_FROM_RANDOM', 2);       // 如果FALSE，否则检查所有可用的面具，面具的count值所需要的，是有随机掩码ID
define('QR_DEFAULT_MASK', 2);           // when QR_FIND_BEST_MASK === false
define('QR_PNG_MAXIMUM_SIZE', 1024);    //最大允许PNG图像宽度（以像素为单位），调整，以确保GD和PHP可以处理这样大的图像。
class QRtools
{
...
class QRspec
{
    ...
    // -----------------------------------------------误差校正码
    //表的误差校正码（RS块）
    //见表12—16（pp.30-36），JIS x0510：2004年
    ...
    /**
     * 对齐方式 对齐模式的位置。
     * 这个数组只包括第二和第三位置对准模式。其余的人，可以计算出它们之间的距离。
    ...
    /**
     * 版本信息模式 -----------------------------------------
     * 版本信息模式（BCH编码）。见附录D中1台（pp.68）JIS x0510:2004。
     * 大小: [QRSPEC_VERSION_MAX - 6]
     * @var array
     */
    ...
    /**
     * 格式信息 --------------------------------------------------
     * 看到测试/ test_qrspec calcformatinfo。C（原qrencode C库）
     * @var array
     */
